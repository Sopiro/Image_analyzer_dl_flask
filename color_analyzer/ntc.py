...
# shades = {i: shades[i] for i in range(0, len(shades))}
...
def hsl(color):
    rgb_ = rgb(color)
    hls = colorsys.rgb_to_hls(rgb_[0] / 255, rgb_[1] / 255, rgb_[2] / 255)
    h = hls[0] * 255
    l = hls[1] * 255
    s = hls[2] * 255
...
for i in range(names.__len__()):
    ...
    names[i] += [rgb_[0], rgb_[1], rgb_[2], hsl_[0], hsl_[1], hsl_[2]]
...
def name(rgb_color):
    ...
    color_ = binascii.hexlify(bytearray(int(c) for c in rgb_color)).decode('ascii').upper()

    for i in range(names.__len__()):
        if color_ == names[i][0]:
            return ["#" + names[i][0], names[i][1], shadergb(names[i][2]), names[i][2], True]

        ndf1 = pow(r - names[i][3], 2) + pow(g - names[i][4], 2) + pow(b - names[i][5], 2)
        ndf2 = abs(pow(h - names[i][6], 2)) + pow(s - names[i][7], 2) + abs(pow(l - names[i][8], 2))
        ndf = ndf1 + ndf2 * 2
    ...
    if cl < 0:
        res = ["#000000", "Invalid Color: " + color_, "#000000", "", False]
    else:
        res = ["#" + names[cl][0], names[cl][1], shadergb(names[cl][2]), names[cl][2], False]

    return res


def name_str(rgb_str):
    return name(rgb(rgb_str))
